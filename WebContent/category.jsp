    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" onClick="Update('category')" id="updateSubmit" class="btn btn-primary">提交更改</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
</div>
...
<div class="modal fade" id="insertModal" tabindex="-1" role="dialog" aria-labelledby="insertModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <span id="familyName" style="display:none"><%out.print(familyName);%></span><!-- 保存当前目名 -->
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" onClick="Insert('category')" id="insertSubmit" class="btn btn-primary">添加</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
  </div>
    ...
    <div class="col-md-4 column">
      ...
      <%for(int i = 0; i<category.size(); i+=COWNUM) //i表示数组的下标
      {
    	  if(category.get(i)!=null){
    		  out.println("<h3><a id=\"valueA"+i+"\" href=\"fishMenu.jsp?categoryName="+category.get(i).getName()+"\">"+category.get(i).getName()+"</a>"
    		  +"<button style=\"display:none\" type=\"button\" onClick=\"Del(this,'category')\" class=\"btn btn-danger btn-xs btn-delete\">删除</button>"
